...
supportMeBtn.addEventListener('click', () => {
  chrome.tabs.create({ url: 'https://buymeacoffee.com/lennacodes' });
});
...
function addDomain() {
  ...
  chrome.storage.local.get('blockedDomains').then((result) => {
    ...
    if (!domains.includes(domain)) {
      domains.push(domain);
      chrome.storage.local.set({ blockedDomains: domains }).then(() => {
        domainInput.value = '';
      });
    }
  });
...
function removeDomain(domain) {
  chrome.storage.local.get('blockedDomains').then((result) => {
    ...
    chrome.storage.local.set({ blockedDomains: updated }).then(() => {
      loadBlockedDomains();
    });
  });
}
...
function loadRedirectRules() {
  chrome.storage.local.get('redirectRules').then((result) => {
...
function addRule() {
  ...
  chrome.storage.local.get('redirectRules').then((result) => {
    ...
    if (!rules.some(r => r.from === from)) {
      rules.push({ from, to });
      chrome.storage.local.set({ redirectRules: rules }).then(() => {
        fromInput.value = '';
        toInput.value = '';
        loadRedirectRules();
      });
